...
- Database: **MySQL** (AwardSpace Hosting) ✅ _Migrated from JSON_
...
├─────────────────────────────────────────────────┤
│  Persistence (MySQL on AwardSpace) ✅           │ ← ที่เก็บจริง
└─────────────────────────────────────────────────┘
```
...
- [x] เขียน Repository ใหม่เป็น `MysqlUserRepository` etc.
- [x] เปลี่ยน Service Container ให้ใช้ MySQL Repo
- [x] เขียน Migration Script ย้ายข้อมูลจาก JSON → MySQL
...
- Phase 7 MySQL migration foundation is completed; deploy requires running schema patch and migration on AwardSpace.
